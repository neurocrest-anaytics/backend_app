...
from app.routers.quotes import get_quote as get_live_price
...
# -------------------- Time & constants --------------------
# ✅ 1. Define timezone (UTC-safe)
MARKET_TZ = dt_timezone.utc

# ✅ 2. Define market hours BEFORE anything uses them
# Example for Indian market (09:15–15:30 IST) = (03:45–10:00 UTC)
MARKET_OPEN = time(3, 45)   # 02:50 IST in UTC
MARKET_CLOSE = time(11, 30)  # 15:30 IST in UTC

# ✅ 3. Define cutoffs *after* MARKET_CLOSE exists
...
def is_after_market_close() -> bool:
    """Check if we are at or after market close"""
    return _now_utc().time() >= MARKET_CLOSE

def is_after_market_close() -> bool:
    now = datetime.now(MARKET_TZ).time()
    """True at/after official EOD cutoff."""
    return _now_utc().time() >= EOD_CUTOFF
...
def _upsert_portfolio_short(
    ...
    now_iso = _now_utc().strftime("%Y-%m-%d %H:%M:%S")
    c.execute(
...
def place_order(order: OrderData):
       ...
       # return {
        #    "error": "Market closed. Orders allowed only between 03:30–10:15 UTC (09:15–15:30 IST)."
       # }
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    try:
        ...
        if not side_buy:
            ...
            if trigger_price == 0:
                if live_price <= 0:
                    live_price = float(order.price or 0)
        ...
        if trigger_price == 0:
            ...
            if not live_price or live_price <= 0:
                raise HTTPException(status_code=400, detail="❌ Live price unavailable — please use LIMIT order")
